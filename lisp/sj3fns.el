;; sj3fns.c support package for Egg
...
;;;  Modified by Kiyoji Ishii at Sony Corporation
;;;
;;;  Email address: tomura@etl.go.jp
;;;                 kiyoji@sm.sony.co.jp

;;;
;;; このファイルは sj3fns.c の機能と sj3-egg.el の機能の調整をする。
;;; sj3-egg.el を load した後でロードする。
;;;

(defvar sj3fns-support-version "1.00")
;;; Last modified date: Wed Jan 30 09:45:24 JST 1991
	...
	(let((jikouho (KKCP:henkan-next number)))
	  (if jikouho
	    ...
	    (progn (beep) (beep) (beep)
		   (notify 
		    "変換候補が多すぎます。sj3fns.c を修正して nemacs を作り直して下さい。")
		   (setcar (nthcdr 2 (aref egg:*kanji-buffer* number))

;;; 89.3.12  modified for Nemacs Ver.3.0 by K.Handa
;;; 89.9.11  modified for Nemacs Ver.3.2 by K.Handa
...
;;; 89.9.7 by K.Handa
(defun delete-rectangle-line (startdelpos begextra endextra)
  (kanji-delete-region
   (if (and (= begextra 1)
	    (= (jcode-type (1- startdelpos)) 2))
       (1- startdelpos)
     startdelpos)
   (if (and (= endextra 1)
	    (= (jcode-type (point)) 1))
...
(defun open-rectangle-line (startpos begextra endextra)
  (let ((column (+ (current-column) begextra endextra)))
    (goto-char startpos)
    ;;; 89.3.12, 89.9.11  patch by K.Handa
    (if (and (= begextra 1)
	     (= (jcode-type (1- startpos)) 2))
	(delete-char -2))
    ;;; end of patch

;; Convert Japanese texinfo files to info files for Egg on Nemacs
;; Modified by Satoru Tomura(tomura@etl.go.jp)
;;
(defconst jtexinfo-version "1.03")
;; Ver.1.03     modified by M.Nishikimi 89-Dec-14
;; Ver.1.02     modified by S.Tomura 89-Dec-12
;;              a bug fixed
;; Ver.1.01     modified by S.Tomura 89-Dec-7
;;              index related functions modified
...
;;; jtexinfo patch by S. Tomura 89-Dec-6
;;;"    (prog1 (buffer-substring start (point))"
;;;"	   (if (eolp) (forward-char 1)))))     "
    (let ((point (point)))
      (goto-char start)
      (prog1 
	  (if (re-search-forward "\\(.*\\)\\[\\(.*\\)\\]" point t)
	      (cons (buffer-substring (match-beginning 1) (match-end 1))
		    (buffer-substring (match-beginning 2) (match-end 2)))
	    (buffer-substring start point))
	(goto-char point)
	(if (eolp) (forward-char 1))))))
;;; end of jtexinfo patch	     
...
(defun texinfo-print-index (file indexelts)
  (while indexelts
    (cond((stringp (nth 0 (car indexelts)))
	  (insert (nth 0 (car indexelts)) "\0") ;;; sort key
	  (insert "* " (nth 0 (car indexelts))
		  ": " (if file (concat "(" file ")") "")
		  (nth 1 (car indexelts)) ".\n"))
	 ((consp (nth 0 (car indexelts)))
	  (insert (cdr (nth 0 (car indexelts))) "\0") ;;; sort key
	  (insert "* " (car (nth 0 (car indexelts)))
		  ": " (if file (concat "(" file ")") "")
		  (nth 1 (car indexelts)) ".\n"))
	 (t
	  ;; index entries from @include'd file
...
(defun texinfo-format-defun-1 (first-p)
  (let ((args (texinfo-format-parse-defun-args))
    ...
    (if (not first-p)
	(delete-char -1))
    ;; patch by M.Nishikimi 89-Dec-14
    ;; original line is

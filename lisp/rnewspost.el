...
;;;; Ver. 18.50b  modified by S. Tomura 89-Jan-25
;;;;              news-make-reply-yank-header added
;;;;              news-yank-original-quoting-indicator and
;;;;              news-yank-original-default-indicator added
;;;; Ver. 18.50a  modified by S. Tomura 88-Jun-25
;;;;              rnews accept FCC: fields
;;;; Ver. 18.50   created by S. Tomura 88-Jun-11
;;;; Nemacs Ver.2.1  
;;;; Ver. 18.47c  modified by S. Tomura 88-May-20
;;;;              news-cancel-news added
;;;; Ver. 18.47b  modified by S. Tomura 88-May-6
;;;;              Distribution: field becomes required field.
;;;; Ver. 18.47a  modified by S. Tomura 88-Feb-8
;;;;              Remote rnews facility added.
...
;;; patch by S. Tomura 89-Jan-25
(defun news-make-reply-yank-header (message-id from)
  (format "In article %s %s writes:\n\n" message-id from))

(defvar news-make-reply-yank-header 'news-make-reply-yank-header)
(defvar news-yank-original-quoting-indicator "|")
(defvar news-yank-original-default-indentation 1)
;;; end of patch 
...
;;;  (mail-yank-original arg)
  (let ((mail-yank-ignored-headers (if (boundp 'news-yank-ignored-headers) news-yank-ignored-headers
				     mail-yank-ignored-headers))
	(mail-yank-original-quoting-indicator news-yank-original-quoting-indicator)
	(mail-yank-original-default-indentation news-yank-original-default-indentation )
	)
    (mail-yank-original arg)
    (setq news-yank-original-quoting-indicator   mail-yank-original-quoting-indicator	
          news-yank-original-default-indentation mail-yank-original-default-indentation))
;;; end of patch
  (exchange-point-and-mark)
;;; patch by S. Tomura 89-Jan-25
;;;  (insert "In article " news-reply-yank-message-id
;;;	  " " news-reply-yank-from " writes:\n\n"))
  (insert (funcall news-make-reply-yank-header 
...
(defvar news-inews-host nil
  "Host name where inews is running. If nil host is local.")
;;; end of patch
...
(defun news-inews ()
  ...
  (let* (newsgroups subject
    ...
    (save-excursion
      ;;; patch by S.Tomura 88-Jun-25
      (goto-char (point-min))
      (search-forward (concat "\n" mail-header-separator "\n"))
      (beginning-of-line 0)
      (let ((case-fold-search t)
	    (delimline (point-marker)))
...
(defun news-cancel-news ()
  "Cancel current news article."
  (interactive)
  (if (y-or-n-p "Are you sure you want to cancel this article? ")
      (let ((newsbuf (current-buffer))
	    (errbuf (get-buffer-create "*Utility Notification*"))
	    message-id distribution newsgroups
	    point)
	(save-restriction
	  (and (not (= 0 (buffer-size))) ;@@real problem is non-existence of
					;@@	of article file
	...
	(save-excursion 
	  (set-buffer errbuf)
	  (let ((point (point)))
	    (message "Canceling this article ....")
	    (if news-inews-host
	     ;;; The arguments of rsh must be quoted.
		(call-process 
			   "rsh" nil errbuf nil
			   news-inews-host
			   news-inews-program
			   "-c" "cancel" (format "'%s'" message-id)
			   "-n" newsgroups
			   "-d" (format  "'%s'" distribution))
	      (call-process
			   news-inews-program nil errbuf nil
			   "-c" "cancel" message-id
			   "-n" newsgroups
			   "-d" distribution))
	    (if (= point (point))
		(progn (set-buffer newsbuf)
		       (let ((buffer-read-only nil))
			 (erase-buffer))
		       (message "Canceling this article .... DONE"))

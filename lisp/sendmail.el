;;;; Ver.18.55a  modified by S. Tomura 89-Oct-24
;;;;             mail-yank-original: universal argument handling
;;;; Ver.18.55   created by S. Tomura 89-Oct-24
;;;; Ver.18.52   created by S. Tomura 89-Mar-16
;;; Nemacs Ver. 3.0
;;;; Ver.18.51a  modified by S. Tomura 88-Sep-2
;;;;             Fcc: checks the file is in buffer or not.
;;;; Ver.18.50   created by S. Tomura 88-Jun-11
;;; Nemacs Ver.2.1   
;;;; Ver.18.47b  modified by S. Tomura 88-Jun-7
;;;;             mail-yank-original is changed to prompt quoting marker.
;;;; Ver.18.47a  modified by S. Tomura 88-Feb-8
;;;;             Remote sendmail facility added.
;;;; Ver.xx.yyc  modified by S. Tomura .....
;;;;	  mail-yank-original-indent-rigidly added.
;;;;
...
;;; patch by S.Tomura 87-Dec-8
(defvar sendmail-host-name nil "Host name where the mail will be sent to.")
;;; end of patch
...
(defun mail-do-fcc (header-end)
  (let (fcc-list
	...
	(while (search-forward "\nFrom " nil t)
	  (forward-char -5)
	  (insert ?>)))
      ;;; patch by S. Tomura 88-Sep-2
      ;;; We must check the file is in buffer or not. If it is in buffer, 
      ;;; something other than append-to-file must be done.
      (let ((l fcc-list))
	(while l
	  (if (get-file-buffer (car l))
	      (progn (beep)
		     (error "Can not do Fcc: file %s exists in buffer."
			       (car l))))
	  (setq l (cdr l))))
      ;;; end of patch 
      (while fcc-list
...
and don't delete any header fields."
  (interactive "P")
;;; patch by S. Tomura 1988-Jun-7, 89-Oct-24
  (if (consp arg)
      nil
    (let ((marker (read-string
		   (concat "Quoting marker (default: " 
			   mail-yank-original-quoting-indicator
			   ") : " ))))
      (if (not (equal marker ""))
	  (setq mail-yank-original-quoting-indicator marker))))
;;; end of patch  
...
;;; patched by S. Tomura 1987-06-12
;;;	  (indent-rigidly start (mark)
;;;			  (if arg (prefix-numeric-value arg) 3)))
	  (mail-yank-original-indent-rigidly start (mark)
		(if arg (prefix-numeric-value arg) 
		    mail-yank-original-default-indentation)
		mail-yank-original-quoting-indicator))
;;; end of patch
...
;;; patched by S. Tomura 1987-06-12
(defvar mail-yank-original-default-indentation 3 " ")
(defvar mail-yank-original-quoting-indicator ">>"  " ")

(defun mail-yank-original-indent-rigidly (start end arg indicator)
  "Indent all lines starting in the region sideways by ARG columns and insert indicator.
Called from a program, takes three arguments, START, END and ARG."
  ...
  (save-excursion
	...
	(insert indicator)
	(indent-to (max 0 (+ indent arg (length indicator))) 0))
      (forward-line 1))
    (move-marker end nil)))
;;; end of patch.

...
;;; ¸ì´Ä¶­¤òÄó¶¡¤¹¤ë¥·¥¹¥Æ¥à¤Ç¤¹¡£¡Ö¤¿¤Þ¤´¡×Âè£²£ÓÈÇ¤Ç¤Ï sj3 ¤Î¤«¤Ê´Á»ú
;;; ÊÑ´¹¥µ¡¼¥Ð¤ò»ÈÍÑ¤·¤Æ¤¤¤Þ¤¹¡£
;;;
...
;;;
;;; ¡ÖÂè£²£ÓÈÇ¡×¤Ë´Ø¤¹¤ëÄó°Æ¡¢Ãî¾ðÊó¤Ï kiyoji@sm.sony.co.jp ¤Ë¤ªÁ÷¤ê²¼¤µ¤¤¡£
;;;

;;;
;;;                      ¢© 141 ÅìµþÅÔÉÊÀî¶èÅì¸ÞÈ¿ÅÄ1-22-1 ¸ÞÈ¿ÅÄ AN ¥Ó¥ë
;;;                      ¥½¥Ë¡¼³ô¼°²ñ¼Ò¥Ñ¡¼¥½¥Ê¥ë¥¤¥ó¥Õ¥©¥á¡¼¥·¥ç¥ó»ö¶ÈËÜÉô
;;;                      ¥Û¡¼¥à¥¤¥ó¥¿¥é¥¯¥Æ¥£¥Ö»ö¶ÈÉô
;;;
;;;                                                ÀÐ°æ À¶¼¡
...
;;;=================================================================== 
;;;
;;; (eval-when (load) (require 'sj3-client))
;;;
(provide 'sj3-egg)
;;;(require 'sj3-client)

(defvar egg-version "1.01" "Version number of this version of Egg. ")
;;; Last modified date: Fri Jul 12 10:29:05 JST 1991

;;;;  ½¤Àµ¥á¥â¡¨¡¨

;;;   Jul-12-91 by K.Ishii
;;;   *bunpo-menu* ¤ò *sj3-bunpo-menu*¡¢*bunpo-code* ¤ò *sj3-bunpo-code*
;;;   ¤ËÊÑ¹¹¤·¤¿¡£(Wnn ¥Ð¡¼¥¸¥ç¥ó¤Î¸å¤Ë load ¤¹¤ë¤È¤«¤Á¹ç¤Ã¤Æ¤·¤Þ¤¦¤¿¤á)

;;;;  Jan-29-91 by K.Ishii
;;;;  sj3serv ¤È¤ÎÄÌ¿®ÍÑ¤Ë½¤Àµ
...
(defun KKCP:error (errorCode &rest form)
  (cond((eq errorCode ':SJ3_SOCK_OPEN_FAIL)
	(notify "EGG: %s ¾å¤Ë SJ3SERV ¤¬¤¢¤ê¤Þ¤»¤ó¡£" (or (get-sj3-host-name) "local"))
	(if debug-on-error
	    (error "EGG: No SJ3SERV on %s is running." (or (get-sj3-host-name) "local"))
	  (error  "EGG: %s ¾å¤Ë SJ3SERV ¤¬¤¢¤ê¤Þ¤»¤ó¡£" (or (get-sj3-host-name) "local"))))
       ((eq errorCode ':SJ3_SERVER_DEAD)
	(notify "EGG: %s ¾å¤ÎSJ3SERV ¤¬»à¤ó¤Ç¤¤¤Þ¤¹¡£" (or (get-sj3-host-name) "local"))
	(if debug-on-error
	    (error "EGG: SJ3SERV on %s is dead." (or (get-sj3-host-name) "local"))
	  (error  "EGG: %s ¾å¤Î SJ3SERV ¤¬»à¤ó¤Ç¤¤¤Þ¤¹¡£" (or (get-sj3-host-name) "local"))))
       ((and (consp errorCode)
...
;;  (let ((result (sj3-bunsetu-kouho-inspect bunsetu-no jikouho-no)))
;;    (cond ((null sj3-error-code) result)
;;	  ((eq sj3-error-code ':sj3-no-connection)
;;	   (EGG:open-sj3)
...
(defun KKCP:dict-info (yomi)
  (let ((result (sj3-server-dict-info yomi)))
    (cond ((null sj3-error-code) result)
	  ((eq sj3-error-code ':sj3-no-connection)
	   (EGG:open-sj3)
	   (KKCP:dict-info yomi))
	  ((null *KKCP:error-flag*) result)
	  (t (KKCP:error sj3-error-code 'KKCP:dict-info yomi)))))
...
(defun egg:setusrstdy (stdy)
  (cond((null (KKCP:file-access stdy 0))
	(notify "³Ø½¬¥Õ¥¡¥¤¥ë %s ¤¬¤¢¤ê¤Þ¤»¤ó¡£" stdy)
	(if (yes-or-no-p (format "³Ø½¬¥Õ¥¡¥¤¥ë %s ¤òºî¤ê¤Þ¤¹¤«¡©" stdy))
	    (if (null (KKCP:make-stdy stdy))
		(error "EGG: setusrstdy failed. : %s" stdy)
	      (notify "³Ø½¬¥Õ¥¡¥¤¥ë %s ¤òºî¤ê¤Þ¤·¤¿¡£" stdy)
...
(defun EGG:open-sj3 ()
  ...
  (setq egg:*sys-dict* nil
	...
	egg:*dict-list* nil
	egg:*dict-menu* nil)
  (notify "¥Û¥¹¥È %s ¤Î SJ3 ¤òµ¯Æ°¤·¤Þ¤·¤¿¡£" (or (get-sj3-host-name) "local"))
...
;;      (progn (sj3-server-dict-save)
;;	     (message "Sj3¤ÎÉÑÅÙ¾ðÊó¡¦¼­½ñ¾ðÊó¤òÂàÈò¤·¤Þ¤·¤¿¡£")
;;	     (sit-for 0)))
  (KKCP:server-close))
...
(defun diced-display (dict-info)
	;;; (values (list (record kanji bunpo dict-no)))
	;;;                         0     1     2
	...
	(l-bunpo
	 (apply 'max
		(mapcar (function(lambda (l)
				   (length (nth 1 (assq (nth 1 l) *sj3-bunpo-code*)))))
			dict-info))))
    (while dict-info
...
(defun diced-add ()
  ...
  (let*((kanji  (read-from-minibuffer "´Á»ú¡§"))
	...
	(progn
	  (KKCP:dict-add dict-no kanji *diced-yomi* bunpo)
	  (notify "¼­½ñ¹àÌÜ¡Ø%s¡Ù(%s: %s)¤ò%s¤ËÅÐÏ¿¤·¤Þ¤·¤¿¡£" 
		  item item-yomi hinshi dict-name)
	  (diced-redisplay)))))
  ...
  (let ((no  0))
    (while (not (eobp))
	  ...
	  (let* ((dict-item (nth no *diced-dict-info*))
	    ...
	    (if (notify-yes-or-no-p "¼­½ñ¹àÌÜ¡Ø%s¡Ù(%s: %s)¤ò%s¤«¤éºï½ü¤·¤Þ¤¹¡£"
				item item-yomi hinshi dict-name)
		(progn
		  (KKCP:dict-delete dict-no kanji *diced-yomi* bunpo)
		  (notify "¼­½ñ¹àÌÜ¡Ø%s¡Ù(%s: %s)¤ò%s¤«¤éºï½ü¤·¤Þ¤·¤¿¡£" item item-yomi hinshi dict-name)
		  ))))
      (setq no (1+ no))
      (forward-line 1)))
  (forward-line -1)
...
;;;
;;; Pure inspect facility (not support for sj3)
;;;
...
;;	  (setq list (cdr list))))
;;      (setq hinshi (nth 1 (assq bunpo *sj3-bunpo-code*)))
;;
;;      (notify-internal

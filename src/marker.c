...
DEFUN ("set-marker-type", Fset_marker_type, Sset_marker_type, 2, 2, 0,
  "Set type of marker MARKER to point_type if POINT_TYPE is not\n\
NIL, otherwise set to conventional type.  Returns MARKER.\n\
...
syms_of_marker ()
{
  ...
  defsubr (&Sset_marker);
/* patch for point_type_marker by K.Handa 89.11.30 */
  defsubr (&Sset_marker_type);
  defsubr (&Smarker_point_type);
/* end of patch */

     ...
     int len;
{
  ...
  register int c;
  char *first_check;
#ifdef NEMACS  /* 89.2.22  by K.Handa */
  char *buf2;
#endif  /* NEMACS */
...
#ifndef NEMACS  /* 90.1.6 by Sanewo */
    len --;
#else
    {
      len --;
      if (current_kanji_flag && JCODE(string[len]))
	/* Last char was 2nd byte of kanji.  So shrink one byte more */
	len --;
    }
#endif  /* NEMACS */
...
#ifdef NEMACS  /* 89.2.9 by K.Handa */
  if (current_kanji_flag
      && kanji_display_code != EUC
      && kanji_display_code != NOCONV) {
    buf2 = get_kanji_work_area(len, kanji_display_code);
    len = convert_kanji_code(len, string, buf2,
			     EUC, kanji_display_code,
			     to_kanji_display, to_ascii_display);
    string = buf2;
  }
#endif  /* NEMACS */
     ...
     int len;
{
  ...
  if (TS_ins_char && current_kanji_flag) {
    int len2 = len;
    while (len2--) OUTPUT1_IF(TS_ins_char);
    if (start)
      output_chars (start, len);
    return;
  }
...
#ifdef NEMACS  /* 89.2.9 by K.Handa */
  if (current_kanji_flag
      && start	/* 89.8.24 by K.Handa */
      && kanji_display_code != EUC
      && kanji_display_code != NOCONV) {
    buf2 = get_kanji_work_area(len, kanji_display_code);
    len = convert_kanji_code(len, start, buf2,
			     EUC, kanji_display_code,
			     to_kanji_display, to_ascii_display);
    start = buf2;
  }
...
#ifdef NEMACS  /* 89.2.9, 89.8.30  by K.Handa */
  if (start && ((!TF_underscore && !TF_hazeltine

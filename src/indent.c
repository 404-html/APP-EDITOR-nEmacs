...
#include "termchar.h"
#include "termopts.h"
#ifdef NEMACS  /* 88.5.30  by K.Handa */
#include "kanji.h"
#endif
...
#ifdef NEMACS  /* 87.5.1  by K.Handa */
/* modified by K.Handa  88.5.30 */
      else if (kflag && EUC_P(c))
	{
	  col++;
	}
#endif  /* NEMACS */
...
#ifndef NEMACS  /* 89.9.7, 90.4.10  by K.Handa */
      else if (c < 040 || c >= 0177)
#else
      else if (c < 040 || (c >= 0177 && !(kflag && EUC_P(c))))
#endif
        col += 3;
    }
...
#ifdef NEMACS  /* 87.7.15  by K.Handa */
  col += point - pos;
#endif
  last_known_column = col;
  last_known_column_point = point;
     ...
     int hscroll, tab_offset;
{
  ...
  for (pos = from; pos < to; pos++)
	...
	hpos++;
#ifdef NEMACS  /* 87.5.1  by K.Handa */
/* modified by K.Handa  87.7.15 */
/* modified by K.Handa  88.5.30, 89.3.10 */
      else if (kflag && EUC_P(c)
	       && pos < ZV-1 && EUC_P(FETCH_CHAR(pos+1)))
	{
	  hpos += 2;
	  pos++;
	}
...
#ifdef NEMACS  /* 87.7.12  by K.Handa */
/* 88.5.30, 89.3.10, 89.12.23  modified by K.Handa */
/* 93/05/07 by S.Okamoto */
      else if (kflag
	       && hpos == width - 1
	       && pos < ZV - 2
	       && EUC_P(FETCH_CHAR (pos+1)) && EUC_P(FETCH_CHAR (pos+2)))
	{
	  if (vpos > tovpos
	      || (vpos == tovpos && hpos >= tohpos))
	    break;

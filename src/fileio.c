...
#ifdef NEMACS  /* 87.6.8  by K.Handa */
#include "kanji.h"
#include "syntax.h"		/* 89.11.17  by K.Handa */
#endif
...
#ifdef OS2

Lisp_Object Qfile_type_from_file_name;

DEFUN ("file-type-from-file-name", Ffile_type_from_file_name,
       Sfile_type_from_file_name, 1, 1, 0,
...
This function always returns \"t\" unless redefined.")
  (filename)
     Lisp_Object filename;
{
  CHECK_STRING (filename, 0);
  return (build_string ("t"));
}

static char check_file_type (Lisp_Object ftype)
{
  char ftc;

  if (XTYPE (ftype) != Lisp_String || XSTRING (ftype)->size != 1 ||
      ((ftc = XSTRING (ftype)->data[0]) != 't' && ftc != 'b'))
    error ("Invalid file type");
  return (ftc);
}

static char file_name_type (Lisp_Object filename)
{
  Lisp_Object ftype;
  ftype = call1 (Qfile_type_from_file_name, filename);
  return (check_file_type (ftype));
}
     ...
     Lisp_Object name, defalt;
{
  ...
  if (p[0] == '~')
      ...
      if (_fngetdrive (p) == 0)
	{
	  ...
	  if (_fngetdrive (newdir) != 0)
	    {
	      o = alloca (strlen (p) + 3);
	      o[0] = _fngetdrive (newdir);
	      o[1] = ':';
	      strcpy (o+2, p);
	      p = o;
	    }
  ...
  if (rel)
      ...
      if (newdir == 0)
	{
      ...
      tlen = strlen (newdir);
      o = alloca (tlen+strlen(p)+10);
      strcpy (o, newdir);
      if (tlen > 0 && newdir[tlen-1] != '/' && newdir[tlen-1] != '\\' &&
	              newdir[tlen-1] != ':')
	  o[tlen++] = '/';
      strcpy (o+tlen, p);
    }
    ...
    o = p;
  if (_abspath (absname, o, sizeof (absname)) != 0)
      return build_string (nm);
  strlwr (absname);
  return build_string (absname);
     ...
     Lisp_Object filename, defdir;
{
  ...
  if (XSTRING (abspath)->size > 1
      ...
      && XSTRING (abspath)->data[XSTRING (abspath)->size - 2] != ':')
#else
  if (XSTRING (abspath)->size > 1
      && XSTRING (abspath)->data[XSTRING (abspath)->size - 1] == '/')
#endif
...
#ifdef OS2
  ifd = open (XSTRING (filename)->data, O_BINARY);
#else				  
...
#ifdef NEMACS /* 89.9.1 by S.Tomura */
Lisp_Object Qfind_kanji_file_input_code;
Lisp_Object Qinvoke_find_kanji_file_input_code;
#endif

#ifndef OS2
/* see original GNU Emacs distribution for previous definition of */
...
If optional third argument TYPE is nil, the file type is chosen based\n\
on the file name. Otherwise, that file type is used.")
  (filename, visit, type)
     Lisp_Object filename, visit, type;
#endif
{
#ifdef OS2
  int left;
...
#ifdef NEMACS /* 89.1.10  by K.Handa */
...
#ifdef OS2
  if (!NULL (type))
    ftc = check_file_type (type);
  else
    ftc = file_name_type (filename);
#endif
  fd = -1;
...
#endif
#ifdef OS2
      || (fd = open (XSTRING (filename)->data,
                     (ftc == 't' ? O_TEXT : O_BINARY)))
#else
      || (fd = open (XSTRING (filename)->data, 0))
#endif
      < 0)
#else
...
#ifdef NEMACS /* 88.1.10  by K.Handa */
  saved_p = GPT_ADDR;
#endif
#ifdef OS2
  left = st.st_size;
...
#ifdef NEMACS /* 89.9.1  by S. Tomura */
  if (!NULL (current_buffer->kanji_flag) &&
      EQ(Fboundp (Qfind_kanji_file_input_code), Qt)) {
    Lisp_Object code, args[5];
    int saved_s1 = GPT-1;
    struct gcpro gcpro1;	/* 90.2.28  patch by S.Abe */
    code = Qnil; 
    args[0] = Qinvoke_find_kanji_file_input_code;
    args[1] = filename ;
    args[2] = visit ;
    args[3] = make_number ( point );
    args[4] = make_number ( point + inserted );
    GCPRO1 (filename);		/* 90.2.28  patch by S.Abe */
    code = Ffuncall (5, args);
    UNGCPRO;			/* 90.2.28  patch by S.Abe */
    if ((GPT-1) != saved_s1) move_gap(saved_s1);
    if (XTYPE(code) != Lisp_Int) code = make_number ( NOCONV );

    switch (XFASTINT (code)) {
    case JIS:
      {
	int reduced_size =
	  inserted - convert_kanji_code(inserted, saved_p, saved_p, JIS, EUC);
/*???????????????????????????????????????*/
	GPT -= reduced_size;
...
/*???????????????????????????????????????*/
	inserted -= reduced_size;
      ...
      break;
    case Shift_JIS:
      convert_kanji_code(inserted, saved_p, saved_p, Shift_JIS, EUC);
      break;
    default:			/* EUC */
      break;
    }
...
#ifdef NEMACS /* 89.9.1 by S.Tomura*/
Lisp_Object Qfind_kanji_file_output_code;
Lisp_Object Qinvoke_find_kanji_file_output_code;
#endif
     ...
     Lisp_Object start, end, filename, append, visit;
{
...
#ifdef OS2
  if (!NULL (current_buffer->file_type))
    ftc = check_file_type (current_buffer->file_type);
  else
    {
      struct gcpro gcpro1;
      GCPRO1 (filename);
  ...
  if (!NULL (append))
#ifdef OS2
    desc = open (fn, O_WRONLY | (ftc == 'b' ? O_BINARY : 0));
#else
...
#ifdef OS2
  setmode (desc, (ftc == 't' ? O_TEXT : O_BINARY));
#endif
 ...
 */
  ...
  if (XINT (start) != XINT (end))
    {
#ifdef NEMACS /* 89.9.1, 89.12.29  by S. Tomura */
     Lisp_Object savep = Qnil;
      Lisp_Object kanji_fileio_code;
  if (!NULL (current_buffer->kanji_flag) &&
      EQ(Fboundp(Qfind_kanji_file_output_code),Qt)) {
    Lisp_Object code, args[6];
    struct gcpro gcpro1;	/* patch by S.Abe */
    code = Qnil; 
    args[0] = Qinvoke_find_kanji_file_output_code;
    args[1] = start;
    args[2] = end; 
    args[3] = filename;
    args[4] = append;
    args[5] = visit;
    GCPRO1 (filename);		/* patch by S.Abe */
    code = Ffuncall (6, args);
    UNGCPRO;			/* patch by S.Abe */
    if (XTYPE(code) != Lisp_Int) code = make_number ( NOCONV );
    savep = Qt;
    kanji_fileio_code = current_buffer->kanji_fileio_code;
    current_buffer->kanji_fileio_code = code;
  }
#endif /* NEMACS */
...
#ifdef NEMACS /* 89.9.3 by S. Tomura */
      if (EQ(savep, Qt)) 
	current_buffer->kanji_fileio_code = kanji_fileio_code;
#endif /* NEMACS */
    }
...
e_write (desc, addr, len)
     ...
     register char *addr;
     register int len;
#ifdef NEMACS /* 88.2.29, 89.2.27  by K.Handa */
/* Big change for handling selective_display. */
{
  unsigned char **buf;
  int maxidx = 0;
  int result = 0, i, idx = 0;
  register unsigned char *p = (unsigned char *)addr;
  register unsigned char *pend = (unsigned char *)addr + len;
  unsigned char *buf2;  /* 89.2.27  by K.Handa */

  if (EQ (current_buffer->selective_display, Qt)) {
    if ((buf = (unsigned char **)malloc((maxidx = 256) * sizeof p))
	== (unsigned char **)0)
      memory_full ();
    while (p < pend) {
      if (*p == '\015') {
	*p = '\n';
	...
	if (idx >= maxidx) {
	  maxidx += 256;
	  if ((buf = (unsigned char **)realloc(buf, (maxidx * sizeof p)))
	      == (unsigned char **)0)
	    memory_full();
	}
      }
...
/* 89.2.27, 89.11.17  patch by K.Handa */
  if (!NULL (current_buffer->kanji_flag)
      && !NULL (current_buffer->kanji_fileio_code)
      && XFASTINT (current_buffer->kanji_fileio_code) != EUC
      && XFASTINT (current_buffer->kanji_fileio_code) != NOCONV) {
				/* We needs code conversion. */
    unsigned char *q = (unsigned char *)kanji_work_area;
    unsigned char *qend = q + KANJI_WORK_SIZE - 9;
    unsigned char c1, c2;
    int kflag = 0;

    p = (unsigned char *)addr;
    pend--;
    switch (XFASTINT(current_buffer->kanji_fileio_code)) {
    case JIS:
      while (p < pend) {
	if (q >= qend) {
	  result = (write(desc, kanji_work_area, (char *)q - kanji_work_area)
		    == (char *)q - kanji_work_area) ? 0 : -1;
	  if (result < 0) break;
	  q = (unsigned char *)kanji_work_area;
	}
	if (kflag) {
	  if (JCODEP(p)) {
	    *q++ = *p++ & 0x7f; *q++ = *p++ & 0x7f;
	  } else {
	    *q++ = ESC_CODE; *q++ = '('; *q++ = to_ascii_fileio;
	    *q++ = *p++;
	    kflag = 0;
	  }
	} else {
	  if (JCODEP(p)) {
	    *q++ = ESC_CODE; *q++ = '$'; *q++ = to_kanji_fileio;
	    *q++ = *p++ & 0x7f; *q++ = *p++ & 0x7f;
	    kflag = 1;
	  } else {
	    *q++ = *p++;
	...
	*q++ = ESC_CODE; *q++ = '('; *q++ = to_ascii_fileio;
    ...
    if (result >= 0) {
      if (p == pend) *q++ = *p++;
      result = (write(desc, kanji_work_area, (char *)q - kanji_work_area)
		== (char *)q - kanji_work_area)
		? 0 : -1;
    }
  } else
    result = (write (desc, addr, len) == len) ? 0 : -1;
/* end of patch */

  if (EQ (current_buffer->selective_display, Qt)) {
    for (i=0; i<idx; i++) *buf[i] = '\015';
    free(buf);
  }
...
#ifdef NEMACS /* 89.9.1 by S.Tomura*/
  Qfind_kanji_file_input_code = intern ("find-kanji-file-input-code");
  staticpro (&Qfind_kanji_file_input_code);
  Qinvoke_find_kanji_file_input_code = intern ("invoke-find-kanji-file-input-code");
  staticpro (&Qinvoke_find_kanji_file_input_code);
  Qfind_kanji_file_output_code = intern ("find-kanji-file-output-code");
  staticpro (&Qfind_kanji_file_output_code);
  Qinvoke_find_kanji_file_output_code = intern ("invoke-find-kanji-file-output-code");
  staticpro (&Qinvoke_find_kanji_file_output_code);
#endif
...
#ifdef OS2
  defsubr (&Sfile_type_from_file_name);
  Qfile_type_from_file_name = intern ("file-type-from-file-name");
  staticpro (&Qfile_type_from_file_name);

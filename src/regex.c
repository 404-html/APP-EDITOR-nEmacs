...
#ifdef NEMACS  /* 89.12.4 by K.Handa */
#define PATFETCH2(c1,c2) \
 {PATFETCH (c2); \
  if (JCODE(c2)) { \
    c1 = (c2 &= 0x7f); \
    PATFETCH (c2); \
...
#ifdef NEMACS  /* 89.11.14  by K.Handa */
  int bf_kanji = !NULL(current_buffer->kanji_flag);
  int jcode_set;
#endif
  ...
  while (p != pend)
	...
	case '[':
#ifdef NEMACS  /* 89.11.14  by K.Handa */
	  {
	    ...
	    if (bf_kanji)
	      while (pp < pend && *pp != ']') {
		if (JCODE((unsigned char)*pp++)) {
		  jcode_set = 1;
		  break;
		}
	      }
	  }
	  if (jcode_set) {	/* If any Japanese code is in the set,
				   we need a large map. */
	    while (b - bufp->buffer
		   > bufp->allocated - 4 - ((1 << 14) / BYTEWIDTH))
	      EXTEND_BUFFER;
	  } else
#endif  /* NEMACS */
	  while (b - bufp->buffer
...
#ifdef NEMACS  /* 89.11.14  by K.Handa */
	  if (jcode_set) {
	    int maxcc = 0, c11, c12, c21, c22;
	    PATPUSH (0);	/* If (!jcode_set) this value is set smaller
				   than 32 [= (1<<BYTEWIDTH)/BYTEWIDTH], and
				   it's a real length of the map.
				   But if (jcode_set) this value is set greater
				   than 33 [= 0x21],
				   and it represents (length_of_map/16). */
	    bzero (b, (1 << 14) / BYTEWIDTH); /* clear the whole map */
	    while (1) {
	      int c11 = 0, c21 = 0;
	      PATFETCH2 (c11,c12);
	      if (c11 > maxcc) maxcc = c11;
	      /* patch by enami@ptgd.sony.co.jp 90/4/10 */
#if 0
	      /* orignal code was */
	      if (c12 == ']' && p != p1 + 1) break;
#else
	      if (c12 == ']' && !c11 && p != p1 + 1) break;
#endif
	      if (*p == '-' && p[1] != ']') {
		unsigned char *bb = (unsigned char *)&b[c11 * 16];
		int ctemp;

		PATFETCH (c22);	/* discard '-' */
		PATFETCH2 (c21,c22);
		if (c21 > maxcc) maxcc = c21;
		if (c11 < c21) {
		  while (c12 < 0x7f) bb[c12 / 8] |= 1 << (c12 % 8), c12++;
		  c12 = 0x21;
		  bb += 16;
		  c11++;
		}
		while (c11 < c21) {
		  bb[0] = bb[1] = 0; bb[2] = 0xfe; /* to start from 0x21 */
		  for (ctemp = 3; ctemp < 15; ctemp++) bb[ctemp] = 255;
		  bb[15] = 0x7f; /* to end at 0x7e */
		  bb += 16;
		  c11++;
		}
		if (c11 == c21)
	  ...
	  if (b[-1] > 32) {	/* in Nemacs, (b[-1] > 32) means jcode_set,
				   and hence can not reduce the map */
	    b += (int)b[-1] * 16;
	    break;
	  }
#endif  /* NEMACS */
...
#ifdef NEMACS  /* 88.6.2, 89.9.19  by K.Handa */
	    case 'c':	
	      laststart = b;
	      PATPUSH (categoryspec);
	      PATFETCH_RAW (c);
	      PATPUSH (category_spec_code[c]);
	      break;
     ...
     struct re_pattern_buffer *bufp;
{
  ...
  int size = bufp->used;
#ifdef NEMACS  /* 89.9.17  by K.Handa */
  register unsigned char *fastmap = (unsigned char *)bufp->fastmap;
  int bf_kanji = !NULL(current_buffer->kanji_flag);
#else
  ...
  while (p)
...
#ifdef emacs
#ifdef NEMACS  /* 88.6.1  by K.Handa */
	case onebyte:
	  for (j = 0; j < (1 << BYTEWIDTH); j++)
	    fastmap[j] = 1;
	  fastmap['\n'] = 0;
	...
	case categoryspec:
	  ...
	  if (CTGSPECIAL(k))
	    ...
	    for (j = 0; j < (1 << BYTEWIDTH); j++)
	      if (category_table[j] == k)
		fastmap[j] = 1;
	  break;

	case notcategoryspec:
	  k = *p++;
	  if (CTGSPECIAL((enum categorycode) k)) {
	    continue;
	  } else
	    for (j = 0; j < (1 << BYTEWIDTH); j++)
	      if (category_table[j] !=  k)
...
re_search_2 (pbufp, string1, size1, string2, size2, startpos, range, regs, mstop)
     struct re_pattern_buffer *pbufp;
#ifdef NEMACS  /* 88.6.9  by K.Handa */
     unsigned char *string1, *string2;	/* We assume that the end of string1 */
     					/* doesn't violate kanji-border. */
#else
     ...
     int mstop;
{
  ...
  while (1)
	  ...
	  if (range > 0)
	    {
	      ...
	      int irange = range;
#ifdef NEMACS  /* 88.6.9  by K.Handa */
	      register int illegal_kanji_border = 0;
#endif
		...
		lim = range - (size1 - startpos);
	      ...
	      if (translate)
		{
#ifdef NEMACS  /* 88.1.5, 88.6.6  by K.Handa */
		  while (range > lim && (!fastmap[translate[*p]]
					 || bf_kanji && illegal_kanji_border))
		    {
		      if (*p++ & 0x80)
			illegal_kanji_border = !illegal_kanji_border;
		      else
			illegal_kanji_border = 0;
		      range--;
		    }
#else
		  while (range > lim && !fastmap[translate[*p++]])
    ...
    advance:
      if (!range) break;
#ifdef NEMACS  /* 88.1.9, 88.6.6, 89.9.15  by K.Handa */
      if (range > 0) {
	if ((startpos >= size1 ? string2 - size1 : string1)[startpos] & 0x80
	    && range >= 2)
	  range -= 2, startpos += 2;
	else
	  range--, startpos++;
...
#ifdef NEMACS  /* 88.1.5,  89.9.17, 89.9.19  by K.Handa */
  int bf_kanji = !NULL(current_buffer->kanji_flag);
  unsigned char *d_1, chr1, chr2;
  enum categorycode ctg1, ctg2;
#endif
  register int mcnt;
...
#ifdef NEMACS  /* 89.9.14, 89.10.22, 90.1.25 */
#define D_INCRE \
  { \
    if (SYNTAX(*d++) == Sjapanese && bf_kanji \
	&& d != dend && SYNTAX(*d) == Sjapanese) d++; \
  }
#define CATEGORY_SET1 \
  { \
    d_1 = d - 1; \
    chr1 = translate ? translate[*d_1] : *d_1; \
    if (d_1 == string1 || d_1 == string2 || SYNTAX(chr1) != Sjapanese) \
      ctg1 = (enum categorycode) SYNTAX(chr1); \
    else { \
      d_1--; \
      ctg1 = CATEGORY(*d_1, chr1); \
    } \
  }
#define CATEGORY_SET2 \
  { \
    d_1 = (d == end1) ? string2 : d; \
    chr1 = translate ? translate[*d_1++] : *d_1++; \
    if (d_1 == dend || SYNTAX(chr1) != Sjapanese) \
      ctg2 = (enum categorycode) SYNTAX(chr1); \
    else \
      ctg2 = CATEGORY(chr1, *d_1); \
  }
#endif
  ...
  while (1)
...
#ifdef NEMACS  /* 89.9.14, 89.10.20  by K.Handa */
	...
	case charset_not:
	  {
	    ...
	    if (*p > 32) {
	      int cc = 0;
	      if (JCODE(c)) cc = c & 0x7f, c = *(d + 1) & 0x7f;
	      if (cc < *p
		  && p[1 + cc * 16 + c / 8] & (1 << (c % 8)))
		not = !not;
	      p += 1 + (int)*p * 16;
	...
	case wordbound:
	  if (d == string1  /* Points to first char */
	    ...
	    break;
#ifdef NEMACS  /* 89.9.14, 89.9.20  by K.Handa */
	  if (bf_kanji) {
	    CATEGORY_SET1;
	    CTGEXTEND(ctg1);
	    CATEGORY_SET2;
	    CTGEXTEND(ctg2);
	    if (CTGHIRA(ctg2) && CTGJWORD(ctg1)) goto fail;
	    if (ctg1 != ctg2 && (CTGWORD(ctg1) || CTGWORD(ctg2))) break;
	    goto fail;
	  }
#endif  /* NEMACS */
	  if ((SYNTAX (d[-1]) == Sword)
	...
	case notwordbound:
	  if (d == string1  /* Points to first char */
	    ...
	    CATEGORY_SET2;
	    CTGEXTEND(ctg2);
	    if (CTGHIRA(ctg2) && CTGJWORD(ctg1)) break;
	    if (ctg1 != ctg2 && (CTGWORD(ctg1) || CTGWORD(ctg2))) goto fail;
	    break;
	  }
#endif  /* NEMACS */
	  if ((SYNTAX (d[-1]) == Sword)
...
#ifdef NEMACS  /* 89.9.14  by K.Handa */
	  if (bf_kanji) {
	    if (d == end2 || (d == end1 && size2 == 0)) goto fail;
	    CATEGORY_SET2;
	    CTGEXTEND(ctg2);
	    if (!CTGWORD(ctg2)) goto fail;
	    if (d == string1) break;
	    CATEGORY_SET1;
	    CTGEXTEND(ctg1);
	    if (ctg1 == ctg2
		|| (CTGHIRA(ctg2) && CTGJWORD(ctg1))) goto fail;
	    break;
	  }
#endif  /* NEMACS */
...
#ifdef NEMACS  /* 89.9.14  by K.Handa */
	  if (bf_kanji) {
	    if (d == string1) goto fail;
	    CATEGORY_SET1;
	    CTGEXTEND(ctg1);
	    if (!CTGWORD(ctg1)) goto fail;
...
#ifdef NEMACS  /* 88.6.2, 89.9.17  by K.Handa */
	case categoryspec:
	  mcnt = *p++;
	  CATEGORY_SET2;
	  PREFETCH;
	  D_INCRE;
	  if (ctg2 != (enum categorycode) mcnt) goto fail;
	  break;
	...
	case wordchar:
#ifdef NEMACS  /* 89.9.17, 89.9.20  by K.Handa */
	  if (bf_kanji) {
	    CATEGORY_SET2;
	    PREFETCH;
	    D_INCRE;
	    if (CTGWORD(ctg2)) break;
	    goto fail;
	  }
#endif
	  mcnt = (int) Sword;
	...
	matchsyntax:
	  ...
	  D_INCRE;
	  if (d - d_1 == 2
	      || SYNTAX (*d_1) != (enum syntaxcode) mcnt) goto fail;
#else
	...
	case notwordchar:
#ifdef NEMACS  /* 89.9.17, 89.10.20  by K.Handa */
	  if (bf_kanji) {
	    CATEGORY_SET2;
	    PREFETCH;
	    D_INCRE;
	...
	case exactn:
	  ...
	  mcnt = *p++;
#ifdef NEMACS	/* 89.10.22  by K.Handa */
	  {  int jcnt = 0;
#endif
	  if (translate)
	    {
	      do
		{
		  PREFETCH;
#ifdef NEMACS	/* 89.10.22  by K.Handa */
		  if (SYNTAX(translate[*d]) == Sjapanese) jcnt++;
		  else jcnt = 0;
#endif
...
#ifdef NEMACS	/* 89.10.22  by K.Handa */
	  if (jcnt % 2 == 1 && SYNTAX(*d) == Sjapanese) goto fail;
	  }
#endif
	  break;
	}

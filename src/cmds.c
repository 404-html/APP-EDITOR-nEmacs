...
#include "buffer.h"
#include "syntax.h"
#ifdef NEMACS  /* 90.3.2  by K.Handa */
#include "kanji.h"
#endif
     ...
     Lisp_Object n;
{
  ...
  SET_PT (point + XINT (n));
#ifdef NEMACS  /* 89.3.10  by K.Handa */
  if (point == saved_point &&  n > 0) point += 2;
#endif  /* NEMACS */
  if (point < BEGV)
    {
      SET_PT (BEGV);
...
#ifdef NEMACS  /* 87.6.10  by K.Handa */
  register int c = (unsigned char)c1;
  unsigned char c2 = FETCH_CHAR (point); /* 90.3.2  by K.Handa */
#else
  register int c = c1;
#endif
...
#ifdef NEMACS  /* 88.1.15  by K.Handa */
  if ((c == ' ' || c == '\n' || c >= 0200)
#else
  if ((c == ' ' || c == '\n')
#endif

  ...
  if (immediate_echo)
    {
      ...
      GCPRO1 (tem1);
      tem1 = make_string (this_command_keys, this_command_key_count);
      tem2 = call1 (Qkey_description, tem1);
      CHECK_STRING (tem2, 1);
      if (XSTRING (tem2)->size + ptr - echobuf <= sizeof echobuf - 4)
	{
	  bcopy (XSTRING (tem2)->data, ptr, XSTRING (tem2)->size);
	  ptr += XSTRING (tem2)->size;
	}
      UNGCPRO;
#else
      char *ptr = echoptr;
...
command_loop_1 ()
{
  ...
  int lose;
  int nonundocount;
#ifdef NEMACS  /* 87.6.6  by K.Handa */
  ...
  while (1)
...
#ifdef NEMACS  /* 87.6.6  by K.Handa */
      kanji1 = keybuf[0];  kanji2 = keybuf[1];
#endif
      ...
      else
	{
	  ...
	  if (NULL (Vprefix_arg) && ! no_direct)
		...
		{
#ifdef NEMACS  /* 89.3.10  by K.Handa */
		  int saved_point = point;
#endif NEMACS
		  lose = FETCH_CHAR (point);
		  SET_PT (point + 1);
#ifdef NEMACS  /* 89.3.10  by K.Handa */
		  if (point == saved_point) point += 2;
#endif
		  if (lose >= ' ' && lose < 0177
...
#ifdef NEMACS  /* 87.6.2  by K.Handa */
   	      else if (EQ (cmd, Qself_insert_command) ||
		       EQ (cmd, Qkanji_self_insert))
#else
	      else if (EQ (cmd, Qself_insert_command))
#endif
		{
		  if (NULL (Vexecuting_macro) &&
		    ...
		    {
#ifdef NEMACS  /* by Y.Ishikawa */
		      if (!DisableUndo && (!nonundocount || nonundocount >= 20))
#else
		  ...
		  lose = (XFASTINT (XWINDOW (selected_window)->last_modified)
		    ...
		    || !NULL (Vexecuting_macro);
#ifdef NEMACS  /* 87.6.6  by K.Handa */
		  if (EQ (cmd, Qkanji_self_insert)) {
/* modified by K.Handa  87.6.13 */
/* modified by K.Handa  87.12.10 */
		    self_insert_internal (kanji1, 1);
		    lose = 0;
		    if (self_insert_internal (kanji2, 0)) {
		      lose = 1;
		      nonundocount = 0;
		    }
		  } else {
	  ...
	  nonundocount = 0;
#ifdef NEMACS  /* by Y.Ishikawa */
	  if (!DisableUndo && NULL (Vprefix_arg))
#else
	  if (NULL (Vprefix_arg))
#endif
	    Fundo_boundary ();
...
#ifdef NEMACS  /* 87.6.2  by K.Handa */
  c &= (meta_key
	|| (!NULL(current_buffer->kanji_flag)
	    && XFASTINT (kanji_input_code) != JIS
	    && XFASTINT (kanji_input_code) != NOCONV) ? 0377 : 0177);
#else
...
#ifdef NEMACS  /* 87.6.2, 89.12.20  by K.Handa */
  if (c == quit_char
	  && (NULL (current_buffer->kanji_flag)
	      || XFASTINT (kanji_input_code) != Shift_JIS))
#else
  if (c == quit_char)
#endif
    {
...
#ifdef NEMACS  /* 88.1.4, 88.5.26  by K.Handa */
  return (c & ((meta_key
		|| (XFASTINT(kanji_input_code) != JIS
		    && XFASTINT(kanji_input_code) != NOCONV)) ? 0377 : 0177));
#else
...
#ifdef NEMACS  /* 87.6.2, 87.12.10  by K.Handa */
  if ((!NULL (current_buffer->kanji_flag)
       && XFASTINT(kanji_input_code) != JIS
       && XFASTINT(kanji_input_code) != NOCONV
       && nextc >= 0x80)
/* modified by K.Handa  88.5.26 */
      || (kanji_JIS_flag && JIS_P(nextc))) {
    switch (XFASTINT (kanji_input_code)) {
    case EUC:
      keybuf[0] = (unsigned char)nextc;
      keybuf[1] = (unsigned char)read_command_char (-1);
      break;
    case Shift_JIS:
      c = read_command_char (-1);	/* patch by t-ishii@srash.sra.junet */
      S2E (nextc, c, keybuf[0], keybuf[1]);
      break;
    case JIS:
      keybuf[0] = (unsigned char)nextc | 0x80;
      keybuf[1] = (unsigned char)read_command_char (-1) | 0x80;
      break;
    }
    read_key_sequence_cmd = Qkanji_self_insert;
/*    keys_prompt = 0; */
    Vquit_flag = Qnil;
    return 2;
  }
...
This feature is only available in Nemacs.");
  DisableUndo = 0;
#endif  /* NEMACS */
}

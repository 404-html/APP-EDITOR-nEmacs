...
#include "buffer.h"
#include "syntax.h"
#ifdef NEMACS  /* 87.12.9  by K. Handa */
#include "kanji.h"
#endif
...
#ifdef NEMACS  /* 89.3.20  by K.Handa, S.Tomura */
Lisp_Object Qkanji_fileio_code;
#endif
...
#ifdef ATTRIBUTE
  b->underlined_region = Qnil;
  b->inversed_region = Qnil;
#endif
/* end of patch */
...
#ifdef NEMACS  /* 89.3.10, 90.1.12  by K.Handa */
  if (kanji_point(XFASTINT(*b)) == 2) XFASTINT(*b)--;
  if (kanji_point(XFASTINT(*e)) == 2) XFASTINT(*e)++;
#endif
...
#ifdef OS2
  buffer_defaults.file_type = Qnil;
#endif
#ifdef NEMACS  /* 87.12.9, 89.2.26  by K.Handa */
  buffer_defaults.kanji_flag = Qt;
  buffer_defaults.kanji_fileio_code = Qnil;
#endif
...
/* patch for attribute of characters by K.Handa 89.11.21 */
#ifdef ATTRIBUTE
  XFASTINT (buffer_local_flags.underlined_region) = -1;
  XFASTINT (buffer_local_flags.inversed_region) = -1;
#endif
/* end of patch */
...
#ifdef OS2
  XFASTINT (buffer_local_flags.file_type) = 0x8000;
#endif
#ifdef NEMACS  /* 88.2.25, 89.12.15  by K.Handa ??????????????????????*/
  XFASTINT (buffer_local_flags.kanji_flag) = 0x4000;
  XFASTINT (buffer_local_flags.kanji_fileio_code) = 0x8000;
#endif
...
#ifdef OS2
  DEFVAR_LISP_NOPRO ("default-file-type",
              &buffer_defaults.file_type,
    "Default file type for buffers that do not override it.\n\
This is the same as (default-value 'file-type).");
#endif
...
#ifdef NEMACS  /* 87.12.9  by K.Handa */
  DEFVAR_LISP_NOPRO ("default-kanji-flag",
	      &buffer_defaults.kanji_flag,
    "Default kanji-flag for buffers that do not override it.\n\
This is the same as (default-value 'kanji-flag).");

  DEFVAR_LISP_NOPRO ("default-kanji-fileio-code",
              &buffer_defaults.kanji_fileio_code,
    "Default kanji-fileio-code for buffers that do not override it.\n\
This is the same as (default-value 'kanji-fileio-code).");
#endif
...
A string is printed verbatim in the mode line except for %-constructs:\n\
  ...
  %c -- print kanji code for fileio, input, and display.\n\
...
0200..0376, are printable.\n\
Nil means display meta chars with backslash and octal digits.\n\
Automatically becomes local when set in any fashion.");

#ifdef OS2
  DEFVAR_PER_BUFFER ("file-type", &current_buffer->file_type,
     "*\"b\" means binary file, \"t\" means text file, nil means default.\n\
Automatically becomes local when set in any fashion.");
#endif
...
#ifdef NEMACS  /* 87.12.9, 89.3.10  by K.Handa */
  DEFVAR_PER_BUFFER ("kanji-flag", &current_buffer->kanji_flag,
    "*non-nil means that this buffer recognizes kanji-code.");

  DEFVAR_PER_BUFFER ("kanji-fileio-code", &current_buffer->kanji_fileio_code,
    "*kanji-code for file and process input/output .\n\
0:EUC/AT&T/DEC  1:Shift-JIS  2:JIS");

  Qkanji_fileio_code = intern("kanji-fileio-code");
#endif

/* patch for attribute of characters by K.Handa 89.11.21 */
#ifdef ATTRIBUTE
  DEFVAR_PER_BUFFER ("underlined-region", &current_buffer->underlined_region,
    "*Underlined region of current buffer.  Should be a list of\n\
LENGTH and several numbers of MARKER_i, where MARKER_i indicates\n\
underline starting point if 'i' is even and underline ending point\n\
if 'i' is odd.  LENGTH is a length of this list.");

  DEFVAR_PER_BUFFER ("inversed-region", &current_buffer->inversed_region,
    "*Inversed region of current buffer.  Should be a list of\n\
LENGTH and several numbers of MARKER_i, where MARKER_i indicates\n\
inverse starting point if 'i' is even and inverse ending point\n\
if 'i' is odd.  LENGTH is a length of this list.");
#endif
/* end of patch */

...
#ifdef NEMACS	/* 89.10.24  by K.Handa */
/* Hook run by `load' before it does really loading.  */
Lisp_Object Vload_hook;
#endif

     ...
     Lisp_Object str, noerror, nomessage, nosuffix;
{
  ...
  struct gcpro gcpro1;
#ifdef NEMACS	/* 89.10.24  by K.Handa */
  struct gcpro gcpro2;		/* 90.6.6 by abe@sunrise.hallab.co.jp */
  Lisp_Object f;
#endif
...
#ifdef OS2
/* Initial State, will probably be changed by openp() */
  opened_binary = 0;
#endif
  ...
  if (XSTRING (str)->size > 0)
    {
#ifdef NEMACS	/* 89.10.24  by K.Handa */
      fd = openp (Vload_path, str, !NULL (nosuffix) ? "" : ".elc:.el:", &f, 0);
#else
...
#ifdef OS2
/* This global Variable has been set by openp() */
  if (opened_binary)
    stream = fdopen(fd, "rb");
  else
    stream = fdopen(fd, "r");
  ...
  *ptr = stream;
  ...
  load_in_progress++;
#ifdef NEMACS	/* 89.10.24, 90.1.26  by K.Handa */
  if (NULL(Vload_hook) || NULL(call2(Vload_hook, f, nomessage)))
#endif	/* NEMACS */
...
/* Under OS/2, compiled lisp files are binary data ! */
#ifdef OS2
		if (strlen (fn) >= 4 &&
		    strcmp (fn + strlen (fn) - 4, ".elc") == 0)
		  {
		    opened_binary = 1;
		    fd = open (fn, O_BINARY, 0);
                  }
                else
		  {
		    opened_binary = 0;
...
otherwise to default specified in by file paths.h when emacs was built.");
  ...
  DEFVAR_LISP ("load-hook", &Vload_hook,
    "Function called, if non-nil, before loading\n\
with two arguments, FILENAME and NOMESSAGE.\n\
If this function returns nil, normal loading procedure is executed.");
#endif

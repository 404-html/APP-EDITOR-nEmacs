...
#include "commands.h"
#include "macros.h"
#ifdef NEMACS  /* 87.6.5  by K.Handa */
#include "kanji.h"
#endif
...
#ifdef ATTRIBUTE
#define APOS(amark) \
  ((XTYPE(amark) == Lisp_Marker && XMARKER (amark)->buffer) \
   ? Fmarker_position (amark) : end)
#endif
...
#ifdef NEMACS  /* 87.6.9  by K.Handa */
  int kflag = !NULL(current_buffer->kanji_flag);
#endif
/* patch for attribute of characters by K.Handa  89.11.21 */
#ifdef ATTRIBUTE
  char attribute_bits = 0;
  char *attrp, *attrstartp;
  Lisp_Object underline, umark, inverse, imark;
  int upos, ipos;
#endif
/* end of patch */
...
#ifdef NEMACS  /* 89.3.9  by K.Handa */
  new_screen->kanji_flag[vpos] = kflag;
#endif
...
/* patch for attribute by K.Handa 89.11.24 */
#ifdef ATTRIBUTE
  attrp = new_screen->attribute[vpos] + 
			(p1 - new_screen->contents[vpos]);
  attrstartp = new_screen->attribute[vpos] + 
			(startp - new_screen->contents[vpos]);
  if (!NULL(current_buffer->underlined_region)
      && XTYPE (current_buffer->underlined_region) == Lisp_Cons) {
    int i = XINT(Fcar (current_buffer->underlined_region)) - 1;
    underline = Fcdr (current_buffer->underlined_region);
    umark = Fcar (underline);
    while (i-- && !NULL(umark) && start >= (upos = APOS (umark))) {
      underline = Fcdr (underline);
      umark = Fcar (underline);
      attribute_bits ^= ATTR_UNDERLINE;
    }
    if (NULL(underline)) upos = end;
  }  else {
    underline = umark = Qnil;
    upos = end;
  }
  if (!NULL(current_buffer->inversed_region)
      && XTYPE (current_buffer->inversed_region) == Lisp_Cons) {
    int i = XINT(Fcar (current_buffer->inversed_region)) - 1;
    inverse = Fcdr (current_buffer->inversed_region);
    imark = Fcar (inverse);
    while (i-- && !NULL(imark) && start >= (ipos = APOS (imark))) {
      inverse = Fcdr (inverse);
      imark = Fcar (inverse);
      attribute_bits ^= ATTR_INVERSE;
    }
    if (NULL(inverse)) ipos = end;
  }  else {
    inverse = imark = Qnil;
    ipos = end;
  }
...
#ifdef NEMACS  /* 87.4.30, 88.1.8, 88.5.30, 89.3.9, 90.3.3  by K.Handa */
      else if (kflag && EUC_P(c) && pos < end - 1 && EUC_P(FETCH_CHAR(pos+1)))
	{
	  if (p1 >= startp)
	    *p1 = c;
	  p1++;
	  ...
	  p = &FETCH_CHAR (pos);
	  if (p1 > startp)
	    *p1 = *p;		/* 90.3.11 by A.Shibata */
	  else if (p1 == startp)
	    *p1 = ' ';
	  p1++;
	  *p++;			/* 90.3.11 by A.Shibata */
	}
#endif  /* NEMACS */
...
#ifdef ATTRIBUTE
      while (attrp < 
	     new_screen->attribute[vpos] + (p1 - new_screen->contents[vpos])) {
	if (attrstartp <= attrp) *attrp = attribute_bits;
	attrp++;
      }
#endif
/* end of patch */
  ...
  if (p1 > endp)
      ...
      if (p1prev)
	{
	  ...
	  if (new_screen->contents[vpos] + hpos < endp)
/* 88.5.30, 89.3.10  by K.Handa */
	  if (kflag && EUC_P(*(unsigned char *)p1prev)
	      && pos < end && EUC_P(*(unsigned char *)(p1prev+1))) {
	    *p1prev = '\\';
	    val.hpos++;
	    pos--;		/* Start the next line with that same 2-byte char */
	    lastpos -= 2;
	    if (point == pos)
  ...
  if (hscroll && lastpos != start)
    {
#ifdef NEMACS  /* 89.3.9  by K.Handa */
      if (!kflag || !EUC_P((unsigned char)*startp))
#endif
      *startp = '$';
      if (p1 <= startp)
	p1 = startp + 1;
     ...
     register int maxwidth;
{
  ...
  switch (c)
    {
#ifdef NEMACS  /* 88.2.1  by K.Handa */
    case 'c':
      decode_mode_spec_buf[0] = NULL(current_buffer->kanji_fileio_code)
	? '-'
	: NSJE[XFASTINT (current_buffer->kanji_fileio_code) % 4];
      decode_mode_spec_buf[1] = NSJE[kanji_input_code % 4];
      decode_mode_spec_buf[2] = NSJE[kanji_display_code % 4];
/* patch by the idea of enami@ptgd.sony.co.jp 1989.11.19 */
      obj = Fget_buffer_process(Fcurrent_buffer ());
      if (!NULL(obj)) obj = Fprocess_kanji_code(obj);
      decode_mode_spec_buf[3] = (NULL(obj)) ? '-' : NSJE[XFASTINT(obj)];
      decode_mode_spec_buf[4] = 0;
      return decode_mode_spec_buf;
/* 90.3.2  patch by S.Tomura */
    case 'C':
      decode_mode_spec_buf[0] = NULL(current_buffer->kanji_fileio_code)
	? '-'
	: NSJE[XFASTINT (current_buffer->kanji_fileio_code) % 4];
      obj = Fget_buffer_process(Fcurrent_buffer ());
      if (!NULL(obj)) obj = Fprocess_kanji_code(obj);
      decode_mode_spec_buf[1] = (NULL(obj)) ? '-' : NSJE[XFASTINT(obj)];
...
#ifdef NEMACS  /* 89.3.5  by K.Handa */
  int kflag = !NULL(current_buffer->kanji_flag);
  new_screen->kanji_flag[vpos] = kflag;
#endif
...
#ifdef NEMACS  /* 87.4.30, 87.7.12, 89.11.18  by K.Handa */
      else if (kflag && EUC_P(c) && EUC_P(*string))
	{
	  if (p1 < start) {
	    p1++;
	    if (p1++ == start) *start = ' ';

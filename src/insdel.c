...
adjust_markers2 (from, to, amount, marker_type)
     register int from, to, amount, marker_type;
/* end of patch */
{
  Lisp_Object marker;
...
/* patch for point_type_marker by S.Okamoto 93.03.06 */
     Lisp_Object string;
...
/* patch for point_type_marker by S.Okamoto 93.03.06 */
insert_from_string_after_markers (string, pos, length)
     Lisp_Object string;
     register int pos, length;
{
  insert_from_string2 (string, pos, length);
     ...
     register int from, to;
{
  ...
  if (to > ZV)
    to = ZV;
#ifdef NEMACS  /* 87.5.1, 89.3.10  by K.Handa */
  if (kanji_point (from) == 2) from--;
  if (kanji_point (to) == 2) to++;
#endif
